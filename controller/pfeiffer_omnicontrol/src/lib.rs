...
pub use lib_utils::{BaseAddress, SensorStatus};
...
impl<T: InstrumentInterface> Channel<T> {
    ...
    /// Get the status of the sensor as a [`SensorStatus`] value.
    pub fn get_status(&mut self) -> Result<SensorStatus, InstrumentError> {
        let base_device = (self.idx + 1) * 10 + 2;
        let cmd = CommandPackage::get_read_pkg(self.base_address, base_device, 41);
        let res = self.query(&cmd)?;
        let data = ReadPackage::try_new(&res)?.get_data_string();
        let status = DataType::UShortInt.parse_to_usize(&data)?;
        status.try_into()
    }

    /// Set the status of the sensor to a given [`SensorStatus`] value.
