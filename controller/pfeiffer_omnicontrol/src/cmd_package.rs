...
impl CommandPackage {
    ...
    /// Create a new write command package. 
    ///
    /// FIXME: The `value` currently needs to be provided as a `&str`. This needs to be corrected
    /// to be automatically handled by the driver according tot he datatype of the parameter.
    ///
    /// Arguments:
    /// - `base_address`: The base address of the instrument, as given in the manual.
    /// - `indicator_address`: The indicator address of the parameter to set, as given in the manual.
    /// - `parameter`: The parameter to set, as given in the manual.
    /// - `value`: The value to set the parameter to, as a string. 
    pub fn get_write_pkg(
        base_address: BaseAddress,
        indicator_address: usize,
        parameter: usize,
        value: &str,
